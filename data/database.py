...
class DataBase:
    def __init__(self):
        ...
        self.async_session: AsyncSession = async_sessionmaker(self.engine, expire_on_commit=False)
    ...
    async def get(self, user_id: int | None=None, all_data: bool=False) -> ScalarResult | None:
        stmt = (select(Users).where(Users.user_id == user_id)) if user_id is not None else select(Users)
        try:
            async with self.async_session.begin() as session:
